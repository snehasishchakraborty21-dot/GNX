...
export const Navbar: React.FC<NavbarProps> = ({ onRequestQuote }) => {
  ...
  const isContactPage = location.pathname === '/contact' || location.pathname === '/find-us';
  const isServicesPage = location.pathname.startsWith('/services') || location.pathname === '/xpress' || location.pathname === '/xfers' || location.pathname === '/xtras';
  const isInternalLightPage = isAboutPage || isContactPage || isServicesPage;
  ...
  const isLightHeader = isScrolled || isInternalLightPage;
