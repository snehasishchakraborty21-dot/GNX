...
export const HowItWorks: React.FC<HowItWorksProps> = ({ onRequestQuote }) => {
  ...
  return (
    <section className="py-24 lg:py-32 bg-[#062B63] text-white border-t border-slate-700/50">
      <div className="w-full max-w-[1440px] mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="flex items-center gap-3">
              ...
              <span className="font-sans text-xs sm:text-sm font-extrabold tracking-[0.25em] text-[#2CB51D] uppercase">
                STREAMLINED ENQUIRY PROCESS
              </span>
            </div>

            <h2 className="font-display text-4xl sm:text-6xl font-black text-white tracking-tight uppercase leading-[0.95]">
              HOW TO REQUEST SERVICE
            </h2>
          </div>

          <p className="font-sans text-slate-200 text-base sm:text-lg max-w-md font-normal">
        ...
        <div className="bg-white border border-slate-200/90 rounded-xs shadow-2xl overflow-hidden grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-200">
          {steps.map((st, idx) => (
            <div key={idx} className="p-8 lg:p-10 space-y-6 flex flex-col justify-between bg-white">
        ...
        {/* Call to Action Bar */}
        <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-8 rounded-xs border border-slate-200/90 shadow-2xl">
          <div>
            <div className="font-display text-2xl font-black text-[#062B63] uppercase">
              READY TO REQUEST SERVICE?
            </div>
            <p className="font-sans text-base text-slate-700 mt-1 font-normal">
